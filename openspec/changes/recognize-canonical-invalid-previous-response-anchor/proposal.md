...
- Close the already-specified durable-owner fencing gap exposed by the cloud regression: a replacement for a reader-retired local session advances the owner epoch or reclaims the already-released row, while an atomic expected-owner check prevents a stale pre-connect lookup from stealing a lease claimed by another replica or process incarnation.
